...
for order in range(1, 20, 4):
    A_spacy = hspacy.get_cylindrical_matrix(2 * n_y, n_order=order, x_range=(-1, 1), y_range=(-1, 1))
    ...
    cylindrical_matrix = A_spacy_list[sel_coil]
    print('Size matrix ', cylindrical_matrix.shape)

    A_approx = hspacy.spacy_approximation(cylindrical_matrix, sel_array.reshape(-1), mask=mask_array.reshape(-1, 1))
    A_approx = A_approx.reshape(n_y, n_x)
    # hplotf.plot_3d_list([A_approx, sel_array], augm='np.real')
    hplotc.ListPlot([A_approx, sel_array], augm='np.real', vmin=(-0.5, 0.5), title=str(order))
