...
from objective_configuration.reconstruction import DRESULT, ACCELERATION_LIST, TYPE_NAMES, MODEL_NAMES, \
    MODEL_COLOR_DICT, MODEL_COLOR_DICT_DARK, INPUT_METRIC_NAMES, TARGET_METRIC_NAMES, FONTSIZE_XTICKS, FONTSIZE_YTICKS, PERCENTAGE_LIST, DRESULT_INFERENCE, \
    MODEL_NAME_DICT, PRETR_SYNTH_ACQ_APPENDIX, FONTSIZE_YLABEL, DRECON, METRIC_NAME_DICT, FONTSIZE_LEGEND, INPUT_METRIC_NAME_DICT, TARGET_METRIC_NAME_DICT
        ...
        if sel_dataframe is not None:
            for jj, i_acc in enumerate(acc_list):
                ...
                if mean_key in sel_acc.keys():
                    ...
                    print(f"{metric} {type} {inference_bool}: {mean_values} {std_values}")
                    plot_bar(ax[jj], x_pos=ii, y_mean=mean_values, y_std=std_values, model_name=i_model)
    #
    for ii, i_ax in enumerate(ax):
        i_ax.set_ylabel(selected_metric_dict[metric], fontsize=FONTSIZE_YLABEL)
        if legend_bool:
...
if __name__ == "__main__":
    ...
    if input_bool:
        selected_metric_names = INPUT_METRIC_NAMES
        selected_metric_dict = INPUT_METRIC_NAME_DICT
        str_prefix = 'input'
    else:
        selected_metric_names = TARGET_METRIC_NAMES
        selected_metric_dict = TARGET_METRIC_NAME_DICT
        str_prefix = 'target'
    ...
    for i_metric in selected_metric_names:
        fig = plot_metric(dict_of_datasets, i_metric, sel_type, acc_list, inference_bool=inference_bool, big=legend_bool)
        if paper_bool:
            if legend_bool:
                dest_path = os.path.join(DRECON, 'Figure 2 - compare models', f'{str_prefix}_{temp_str}_compare_model_{i_metric}_legend.png')
            else:
                dest_path = os.path.join(DRECON, 'Figure 2 - compare models', f'{str_prefix}_{temp_str}_compare_model_{i_metric}.png')
        else:
            dest_path = os.path.join(selected_path, f'{str_prefix}_metric_per_model_100p_{i_metric}_{sel_type}.png')
