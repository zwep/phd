...
"""
Diluated version of Rev Blocks
"""
class DilatedRevBlockFunction(Function):
    @staticmethod
    def residual(x, in_channels, out_channels, params, buffers, training,
        ...
        out = F.conv_transpose2d(out, params[-6], params[-5], stride, padding=padding,
                       dilation=dilation)

        out = F.batch_norm(out, buffers[-2], buffers[-1], params[-4],
                           params[-3], training)
        out = F.relu(out)
        out = F.conv_transpose2d(out, params[-2], params[-1], stride=1, padding=1, dilation=1)
    ...
    def _forward(x, in_channels, out_channels, training, stride, padding,
        ...
        with torch.no_grad():
            ...
            f_x2 = DilatedRevBlockFunction.residual(
                x2,
                in_channels,
                out_channels,
                f_params,
            ...
            g_y1 = DilatedRevBlockFunction.residual(
                y1,
                out_channels,
                out_channels,
                g_params,
    ...
    def _backward(output, in_channels, out_channels, f_params, f_buffs,
        ...
        with torch.no_grad():
            ...
            x2 = y2 - DilatedRevBlockFunction.residual(
                y1,
                out_channels,
                out_channels,
                g_params,
            ...
            x1 = y1 - DilatedRevBlockFunction.residual(
                x2,
                in_channels,
                out_channels,
                f_params,
    ...
    def forward(ctx, x, in_channels, out_channels, training, stride, padding,
        ...
        y = DilatedRevBlockFunction._forward(
            x,
            in_channels,
            out_channels,
            training,
    ...
    def backward(ctx, grad_out):
        ...
        if ctx.load_input:
            ...
            x = DilatedRevBlockFunction._backward(
                output,
                in_channels,
                out_channels,
        ...
        dx, dfw, dgw = DilatedRevBlockFunction._grad(
            x,
            grad_out,
            in_channels,
            out_channels,
...
class DilutedRevBlock(nn.Module):
    ...
    def forward(self, x):
        return DilatedRevBlockFunction.apply(
            x,
            self.in_channels,
            self.out_channels,
            self.training,
...
class RevNet(nn.Module):
    def __init__(self,
        ...
        # Input layer
        self.layers.append(nn.Conv2d(in_channels=3, out_channels=filters[0], kernel_size=3, padding=1))
        self.layers.append(nn.BatchNorm2d(num_features=filters[0]))
...
"""
class RevImgNet(nn.Module):
    def __init__(self, input_dict, bottleneck=False, **kwargs):
        """
        Args:
            input_dict contains all the configuration parameters of this thing..
        ...
        super(RevImgNet, self).__init__()
        self.name = self.__class__.__name__
        self.model_config = input_dict
        units = self.model_config["units"]
        first_layer_in_channels = self.model_config["in_channels"]
        final_layer_dimension_in = self.model_config["final_dense_layer_in"]
        final_layer_dimension_out = self.model_config["final_dense_layer_out"]
        self.filters = self.model_config["filters"]
        self.final_out_channel = self.model_config["final_out_channel"]
        strides = self.model_config["strides"]
        self.n_pixels = self.model_config["n_pixels"]
        ...
        # Input layer
        self.layers.append(nn.Conv2d(in_channels=first_layer_in_channels, out_channels=self.filters[0],
                                     kernel_size=3, padding=1))
        self.layers.append(nn.BatchNorm2d(self.filters[0]))

        for i, group_i in enumerate(units):
            self.layers.append(self.Reversible(in_channels=self.filters[i], out_channels=self.filters[i + 1],
                                               stride=strides[i], no_activation=True, activations=self.activations))
        ...
        self.fc = nn.Linear(final_layer_dimension_in, final_layer_dimension_out)
    ...
    def forward(self, x):
        ...
        # Save last output for backward
        self.activations.append(x.data)
        final_reshape_size = int(x.size()[-2] * x.size()[-1] * self.filters[-1])
        right_size = final_reshape_size // self.n_pixels
        left_size = self.n_pixels
        x = x.view(-1, left_size, right_size)
        x = self.fc(x)
        x = x.view(-1, self.final_out_channel, left_size, self.n_pixels)
        return x
...
if __name__ == "__main__":
    ...
    A = RevBlock(in_channels=3, out_channels=50, activations=[])
    A._parameters.keys()
    A._buffers.keys()


    A = DilutedRevBlock(in_channels=3, out_channels=50, activations=[])
    A._parameters.keys()
    A._buffers.keys()
    ...
    model = RevNet(units=[3, 3, 3],
                   ...
                   strides=[1, 2, 2],
                   classes=10)
    ts.summary(model, (1, 32, 32))

    input_dict = {"units": [3, 3, 3], "in_channels": 3, "final_dense_layer_in": 128, "final_dense_layer_out": 32*3,
                  "filters": [32, 32, 64, 64], "strides": [1, 2, 2], "n_pixels": 32, "final_out_channel": 3}
    model_img = RevImgNet(input_dict)
    model_img.eval()
    A_in = Variable(torch.FloatTensor(12, 3, 32, 32).normal_(0,1))
    model_img(A_in).shape
    ts.summary(model_img, (1, 32, 32))
