...
class DataConsistencyInKspace(nn.Module):
    ...
    def perform(self, img, k0, mask):
        ...
        if c == 1:
            img = torch.cat((img, torch.zeros(q, 1, ix, iy).to(img.device)), 1)

        if ix < nx or iy < ny:
            img = torch.nn.functional.pad(img, (0, nx - ix, 0, ny - iy))

        _, _, x, y = img.shape
        assert (x == nx and y == ny)

        if img.dim() == 4:  # input is 2D
            img = img.permute(0, 2, 3, 1)
            k0 = k0.permute(0, 2, 3, 1)
            mask = mask.permute(0, 2, 3, 1)
        elif img.dim() == 5:  # input is 3D
            img = img.permute(0, 4, 2, 3, 1)
            k0 = k0.permute(0, 4, 2, 3, 1)
            mask = mask.permute(0, 4, 2, 3, 1)

        k = torch.fft.fft(img, 2, normalized=self.normalized)
        ...
        return torch.sqrt(x_res[:, 0, :ix, :iy] ** 2 + x_res[:, 1, :ix, :iy] ** 2).unsqueeze(1)
...
class dAUTOMAPWithRef(nn.Module):
    """
    Pytorch implementation of dAUTOMAP
    ...
    def __init__(self, input_shape, output_shape, tfx_params, tfx_params2=None):
        super(dAUTOMAPWithRef, self).__init__()
        self.input_shape = input_shape
        self.output_shape = output_shape
        ...
        self.domain_transform = GeneralisedIFT2Layer(**tfx_params)
        self.domain_transform2 = GeneralisedIFT2Layer(**tfx_params2)
        self.ref_refinement_block = get_refinement_block('automap_scae', input_shape[0], output_shape[0])
        # self.consistency = DataConsistencyInKspace(norm=None)
        self.consistency = DataConsistencyInKspace()
    ...
    def forward(self, x, m, mask):
        ...
        x_mapped = F.tanh(x_mapped)
        x_mapped2 = self.domain_transform2(x_mapped)
        x_mapped2 = F.tanh(x_mapped2)[:, :, :224, :224]
        recon = self.ref_refinement_block(x_mapped2)
        out = self.consistency(recon, m, mask)
        return out
