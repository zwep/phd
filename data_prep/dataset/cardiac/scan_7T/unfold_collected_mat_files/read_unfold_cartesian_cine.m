...
for i_line = starting_line:length(all_lines)
    ...
    data_dir = split(2);
    folded_file = fullfile(data_dir, split(3));
    sense_file = fullfile(data_dir, split(4));
    ...
    file_name = folded_file(1:end-4)
    target_file = char(fullfile(target_location_path, file_name));

    recon_obj = MRecon(folded_file);
    ...
    if check_data_typ
        if isempty(recon_obj.Parameter.Scan.ScanType)
            ...
            S = MRsense(sense_file, folded_file);
            ...
            recon_obj.Parameter.Recon.Sensitivities = S;
            recon_obj.Perform
