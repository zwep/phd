...
import os
import data_prep.registration.RegistrationProcess_reverse as RegistrationProcess_reverse
...
if remote:
    ...
    dest_dir = '/local_scratch/sharreve/mri_data/reverse_registrated_h5'
else:
    ...
    dest_dir = '/home/bugger/Documents/data/test_clinic_registration/reverse_registrated_h5'
...
list_b1_field = [os.path.join(ddata_b1p, x) for x in os.listdir(ddata_b1p) if x.endswith('mat')]
...
for i_patient in selected_prostate_list:
    ...
    patient_file_list = [os.path.join(patient_path, x) for x in os.listdir(patient_path)]
    ...
    for i_file in patient_file_list:
        ...
        patient_file_shape_list.append(temp_shape + (i_file, ))

    # Now we sort this array.. and reverse the ordering to decensing
    temp_sel = sorted(patient_file_shape_list, key=lambda x: x[0])[::-1][:max_acq_per_patient]
    ...
    for sel_b1_file in selected_b1_list:
        regproc_obj = RegistrationProcess_reverse.RegistrationProcessReverse(patient_files=sel_patient_file_list,
