...
for anatomy in 'mixed'; do
  ...
  name="${name}_${anatomy}"
  output_path="${MODEL_RUN}/${model_name}_PRETR_ACQ/${percentage}p"
  # train_oot="${DDATA}/${anatomy}/train_${percentage}/input"
  if ${DEBUG}; then
    config_path="${MODEL_RUN}/${model_name}_PRETR_ACQ/config/config_${percentage}p_${anatomy}_debug.yaml"
    val_root="${DDATA}/${anatomy}/validation_1/input"
    train_root="${DDATA}/${anatomy}/validation_1/input"
  else
    config_path="${MODEL_RUN}/${model_name}_PRETR_ACQ/config/config_${percentage}p_${anatomy}.yaml"
