...
case "$model_name" in
  varnet)
    # Eigenlijk zou 'latest model' pakken NOG beter zijn.
    # base_ckpt="${PRETRAINED}/varnet_PRETR_SYNTH/${percentage_model}p/train_mixed/model_4999.pt"
    base_dir="${PRETRAINED}/varnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  base_conjgradnet)
    # base_ckpt="${PRETRAINED}/base_conjgradnet_PRETR_SYNTH/100p/train_mixed/model_4999.pt"
    base_dir="${PRETRAINED}/base_conjgradnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  rim)
    # base_ckpt="${PRETRAINED}/rim/model_89000.pt"
    base_dir="${PRETRAINED}/rim_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  multidomainnet)
    # base_ckpt="${PRETRAINED}/multidomainnet/model_50000.pt"
    base_dir="${PRETRAINED}/multidomainnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  xpdnet)
    # base_ckpt="${PRETRAINED}/xpdnet_PRETR_SYNTH/100p/train_mixed/model_4999.pt"
    base_dir="${PRETRAINED}/xpdnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  recurrentvarnet)
    # base_ckpt="${PRETRAINED}/recurrentvarnet/model_148500.pt"
    base_dir="${PRETRAINED}/recurrentvarnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  jointicnet)
    # base_ckpt="${PRETRAINED}/jointicnet/model_43000.pt"
    base_dir="${PRETRAINED}/jointicnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  kikinet)
    base_dir="${PRETRAINED}/kikinet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  base_iterdualnet)
    # base_ckpt="${PRETRAINED}/base_iterdualnet/model_33500.pt"
    base_dir="${PRETRAINED}/base_iterdualnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  unet | unet_test)
    # base_ckpt="${PRETRAINED}/unet_PRETR_SYNTH/100p/train_mixed/model_4999.pt"
    base_dir="${PRETRAINED}/unet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  lpdnet)
    # base_ckpt="${PRETRAINED}/lpdnet/model_96000.pt"
    base_dir="${PRETRAINED}/lpdnet_PRETR_SYNTH/${percentage_model}p/train_mixed"
    base_ckpt=$(find_model_file "${base_dir}")
    ;;
  *)
...
for anatomy in 'mixed'; do
  ...
  name="${name}_${anatomy}"
  # This still refers to the interptation of amount of 7T data used for training
  output_path="${MODEL_RUN}/${model_name}_PRETR_SYNTH_ACQ/${percentage}p"
  # train_oot="${DDATA}/${anatomy}/train_${percentage}/input"
  if ${DEBUG}; then
    config_path="${MODEL_RUN}/${model_name}_PRETR_SYNTH_ACQ/config/config_${percentage}p_${anatomy}_debug.yaml"
    val_root="${DDATA}/${anatomy}/validation_1/input"
    train_root="${DDATA}/${anatomy}/validation_1/input"
  else
    config_path="${MODEL_RUN}/${model_name}_PRETR_SYNTH_ACQ/config/config_${percentage}p_${anatomy}.yaml"
