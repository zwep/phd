...
case "$model_name" in
  ...
  lpdnet)
    ...
    ;;
  *)
    echo "Invalid model_name. Please provide a valid model_name: $model_name"
    exit 1
    ;;
esac
...
for anatomy in 'mixed'; do
  ...
  name="${name}_${anatomy}"
  config_path="${MODEL_RUN}/${model_name}_CIRCUS/config/config_${percentage}p_${anatomy}.yaml"
  output_path="${MODEL_RUN}/${model_name}_CIRCUS/${percentage}p"
